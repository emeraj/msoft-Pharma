        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Substitute Medicines" maxWidth="max-w-2xl">
            <div className="space-y-6">
                <div className="bg-indigo-50 dark:bg-indigo-900/30 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800">
                    ...
                    <p className="text-xs text-slate-500 mt-1 uppercase">Finding alternatives for: <span className="font-bold text-indigo-600">{product.name}</span></p>
                </div>
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto">
                        ...
                        return (
                            <div key={sub.id} className="flex justify-between items-center p-4 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-xl hover:shadow-md transition-shadow group">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">{sub.company} | 1*{sub.unitsPerStrip || 1}</p>
                                </div>
                                <div className="text-right flex items-center gap-4">
                                    <div>
                                        ...
                                        {sub.batches.length > 0 && (
                                            <p className="text-xs text-slate-400 mt-0.5">Rate: ₹{(sub.batches[0].saleRate || sub.batches[0].mrp).toFixed(2)}</p>
                                        )}
                                    </div>
                                    <button 
                                        onClick={() => firstValidBatch && onSelect(sub, firstValidBatch)}
                                        disabled={totalStock <= 0}
                                        className="bg-indigo-100 text-indigo-700 p-2 rounded-lg hover:bg-indigo-600 hover:text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                                    >
                    ...
                    {substitutes.length === 0 && (
                        <div className="py-12 text-center text-slate-400 italic">
                            No other medicines found with similar composition.
                        </div>
                    )}
                </div>
