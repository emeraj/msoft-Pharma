        ...
        <Card title="Bill Summary" className="sticky top-20">
            <div className="space-y-4">
                <div className={`pb-4 border-b dark:border-slate-700 ${systemConfig.maintainCustomerLedger && systemConfig.enableSalesman ? 'grid grid-cols-2 gap-4' : ''}`}>
                    {systemConfig.maintainCustomerLedger && (<div><label className="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-2">Payment Mode</label><div className="flex gap-2 flex-wrap"><label className="flex items-center gap-1 cursor-pointer">
                        {/* Fix: Added missing closing parenthesis to setPaymentMode calls */}
                        <input type="radio" name="paymentMode" value="Cash" checked={paymentMode === 'Cash'} onChange={() => setPaymentMode('Cash')} className="h-4 w-4 text-green-600 focus:ring-green-500 border-slate-300 rounded-full" /> <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Cash</span></label><label className="flex items-center gap-1 cursor-pointer">
                        <input type="radio" name="paymentMode" value="Credit" checked={paymentMode === 'Credit'} onChange={() => setPaymentMode('Credit')} className="h-4 w-4 text-red-600 focus:ring-red-500 border-slate-300 rounded-full" /> <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Credit</span></label></div></div>)}
