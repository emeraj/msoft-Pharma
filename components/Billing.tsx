...
import { TrashIcon, SwitchHorizontalIcon, PencilIcon, CameraIcon, PrinterIcon, CheckCircleIcon, ShareIcon, HomeIcon, PlusIcon, UserCircleIcon, ExclamationTriangleIcon } from './icons/Icons';
...
// ... (Generate ESC/POS, hex, printing helpers maintained - collapsed for brevity) ...
...
const generateEscPosBill = (bill: Bill, profile: CompanyProfile, config: SystemConfig): number[] => {
    ...
    addBytes([ESC, 69, 1]); 
    addText("Item".padEnd(col1W) + "Qty".padStart(col2W) + "Rate".padStart(col3W) + "Amount".padStart(col4W) + "\n");
    addBytes([ESC, 69, 0]); 
    addText('-'.repeat(PRINTER_WIDTH) + '\n');
    bill.items.forEach((item, index) => {
        ...
        addText(" ".repeat(col1W) + qty.padStart(col2W) + rate.padStart(col3W) + amount.padStart(col4W) + "\n");
    });
    addText('-'.repeat(PRINTER_WIDTH) + '\n');
...
const printViaWebBluetooth = async (data: Uint8Array, printerId?: string) => {
    ...
    if (!device) {
        try {
            device = await nav.bluetooth.requestDevice({ acceptAllDevices: true, optionalServices: ['000018f0-0000-1000-8000-00805f9b34fb'] });
        } catch (e) { throw new Error("Device selection cancelled or failed."); }
    }
...
const SubstituteModal: React.FC<{ isOpen: boolean; onClose: () => void; sourceProduct: Product | null; substitutes: Product[]; onAddToCart: (product: Product, batch: Batch) => void; }> = ({ isOpen, onClose, sourceProduct, substitutes, onAddToCart }) => {
                                ...
                                <ul className="mt-2 space-y-1">
                                    {product.batches.filter(b => b.stock > 0).map(batch => (
                                        <li key={batch.id} className="flex justify-between items-center text-sm p-2 bg-slate-50 dark:bg-slate-700/50 rounded">
                                            <div><span>Batch: <span className="font-medium">{batch.batchNumber}</span></span><span className="ml-3">Exp: {batch.expiryDate}</span></div>
                                            <div className="flex items-center gap-4"><span>MRP: <span className="font-medium">₹{batch.mrp.toFixed(2)}</span></span><span className="text-green-600 dark:text-green-400">Stock: {batch.stock}</span><button onClick={() => { onAddToCart(product, batch); onClose(); }} className="px-3 py-1 bg-indigo-600 text-white text-xs font-semibold rounded hover:bg-indigo-700">Add</button></div>
                                        </li>
                                    ))}
                                </ul>
                            </div>
...
interface EditBillItemModalProps { isOpen: boolean; onClose: () => void; item: CartItem; maxStock: number; onUpdate: (batchId: string, updates: { mrp: number, stripQty: number, looseQty: number }) => void; systemConfig: SystemConfig; }
const EditBillItemModal: React.FC<EditBillItemModalProps> = ({ isOpen, onClose, item, maxStock, onUpdate, systemConfig }) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => { const { name, value } = e.target; setFormState(prev => ({ ...prev, [name]: parseFloat(value) || 0 })); };
    const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); const totalRequested = (formState.stripQty * unitsPerStrip) + formState.looseQty; if (totalRequested <= 0) { alert("Quantity must be greater than 0"); return; } if (totalRequested > maxStock) { alert(`Not enough stock. Available: ${maxStock} units`); return; } if (formState.mrp <= 0) { alert("MRP must be greater than 0"); return; } onUpdate(item.batchId, { mrp: formState.mrp, stripQty: isPharmaMode && unitsPerStrip > 1 ? formState.stripQty : 0, looseQty: formState.looseQty }); onClose(); };
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Edit Item">
            <form onSubmit={handleSubmit} className="space-y-4">
                <div><p className="font-semibold text-slate-800 dark:text-slate-200">{item.productName}</p><p className="text-sm text-slate-600 dark:text-slate-400">Batch: {item.batchNumber} | Exp: {item.expiryDate}</p><p className="text-sm text-slate-600 dark:text-slate-400">Available Stock: {formatStock(maxStock, unitsPerStrip)}</p></div>
                <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">MRP</label><input type="number" name="mrp" value={formState.mrp} onChange={handleChange} className={`${modalInputStyle} ${!isMrpEditable ? 'bg-slate-200 dark:bg-slate-700 cursor-not-allowed opacity-70' : ''}`} step="0.01" min="0.01" readOnly={!isMrpEditable}/></div>
                    {isPharmaMode && unitsPerStrip > 1 ? ( <> <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Strips</label><input type="number" name="stripQty" value={formState.stripQty} onChange={handleChange} className={modalInputStyle} min="0"/></div> <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Loose Tabs</label><input type="number" name="looseQty" value={formState.looseQty} onChange={handleChange} className={modalInputStyle} min="0"/></div> </> ) : ( <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Quantity</label><input type="number" name="looseQty" value={formState.looseQty} onChange={handleChange} className={modalInputStyle} min="1"/></div> )}
                </div>
                <div className="flex justify-end gap-2 pt-4 border-t dark:border-slate-700 mt-4"><button type="button" onClick={onClose} className="px-4 py-2 bg-slate-200 dark:bg-slate-600 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-500 text-slate-700 dark:text-slate-200">Cancel</button><button type="submit" className="px-6 py-2 bg-indigo-600 text-white rounded-lg shadow hover:bg-indigo-700">Update</button></div>
            </form>
        </Modal>
    );
};
...
const OrderSuccessModal: React.FC<{ isOpen: boolean; onClose: () => void; bill: Bill | null; timeTaken: number; onPrint: () => void; onCreateNew: () => void; onEditOrder: () => void; companyProfile: CompanyProfile; }> = ({ isOpen, onClose, bill, timeTaken, onPrint, onCreateNew, onEditOrder, companyProfile }) => {
    ...
    const shareToWhatsApp = () => { if (!mobileNumber || !mobileNumber.trim()) { alert("Mobile Number is mandatory to share on WhatsApp."); return; } const text = `*TAX INVOICE*\n${companyProfile.name}\nBill No: ${bill.billNumber}\nDate: ${new Date(bill.date).toLocaleDateString()}\n\n*Items:*\n${bill.items.map(i => `${i.productName} x ${i.quantity} = ${i.total.toFixed(2)}`).join('\n')}\n\n*Total: ₹${bill.grandTotal.toFixed(2)}*\n\nThank you!`; let url = ''; const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent); let cleanNumber = mobileNumber.replace(/\D/g, ''); if (cleanNumber.length === 10) { cleanNumber = '91' + cleanNumber; } if (isMobile) { url = `https://api.whatsapp.com/send?phone=${cleanNumber}&text=${encodeURIComponent(text)}`; } else { url = `https://web.whatsapp.com/send?phone=${cleanNumber}&text=${encodeURIComponent(text)}`; } window.open(url, '_blank'); };
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[70] flex justify-center items-center">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-fade-in-up">
                <div className="p-6 flex flex-col items-center text-center">
                    {upiId && ( <div className="mb-4 bg-white p-2 rounded-lg border-2 border-slate-200 dark:border-slate-600"> <img src={qrCodeUrl} alt="Payment QR" className="w-40 h-40 object-contain"/> <p className="text-xs text-slate-500 mt-1 font-medium">Scan to Pay ₹{amount}</p> </div> )}
                    ...
                    <div className="w-full mb-4"><input type="tel" value={mobileNumber} onChange={(e) => setMobileNumber(e.target.value)} placeholder="Customer WhatsApp No.*" className="w-full px-4 py-2 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-green-500 outline-none transition-all text-center placeholder-slate-500 font-medium text-lg tracking-wide"/></div>
                    <div className="w-full space-y-3">
                        <div className="flex gap-3"><button onClick={shareToWhatsApp} className="flex-1 flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white py-3 rounded-lg font-semibold transition-colors"><ShareIcon className="h-5 w-5" /> {/iPhone|iPad|iPod|Android/i.test(navigator.userAgent) ? 'Continue to WhatsApp' : 'Continue to WhatsApp Web'}</button><button onClick={onPrint} className="flex-1 flex items-center justify-center gap-2 bg-black hover:bg-gray-800 text-white py-3 rounded-lg font-semibold transition-colors"><PrinterIcon className="h-5 w-5" /> Print Bill</button></div>
...
const Billing: React.FC<BillingProps> = ({ products, bills, customers, salesmen, onGenerateBill, companyProfile, systemConfig, editingBill, onUpdateBill, onCancelEdit, onAddCustomer, onAddSalesman }) => {
  ...
  const searchResults = useMemo(() => {
    ...
    return products.filter(p => (p.name.toLowerCase().includes(lowerSearchTerm) || (!isPharmaMode && p.barcode && p.barcode.includes(lowerSearchTerm))) && p.batches.some(b => b.stock > 0 && (isPharmaMode ? getExpiryDate(b.expiryDate) >= today : true))).slice(0, 10);
  }, [searchTerm, products, today, isPharmaMode]);
  
  const navigableBatchesByProduct = useMemo(() => {
    return searchResults.map(p => p.batches.filter(b => b.stock > 0 && (isPharmaMode ? getExpiryDate(b.expiryDate) >= today : true)).sort((a, b) => isPharmaMode ? (getExpiryDate(a.expiryDate).getTime() - getExpiryDate(b.expiryDate).getTime()) : 0));
  ...
  // ... (print functions maintained) ...
  const executePrint = useCallback(async (bill: Bill, printer: PrinterProfile, forceReset = false) => {
    const doReset = () => { setCart([]); setCustomerName(''); setSelectedCustomer(null); setDoctorName(''); setPaymentMode('Cash'); setSelectedSalesmanId(''); if (onCancelEdit && isEditing) { onCancelEdit(); } setShouldResetAfterPrint(false); };
    const shouldReset = forceReset || shouldResetAfterPrint;
    if (printer.connectionType === 'rawbt') { const data = generateEscPosBill(bill, companyProfile, systemConfig); const base64 = btoa(data.reduce((acc, byte) => acc + String.fromCharCode(byte), '')); const isAndroid = /Android/i.test(navigator.userAgent); if (isAndroid) { const intentUrl = `intent:base64,${base64}#Intent;scheme=rawbt;package=ru.a402d.rawbtprinter;end;`; const a = document.createElement('a'); a.href = intentUrl; a.style.display = 'none'; document.body.appendChild(a); a.click(); document.body.removeChild(a); } else { window.location.href = `rawbt:base64,${base64}`; } if (shouldReset) { setTimeout(doReset, 1000); } return; }
    ...
    if (printWindow) { const style = printWindow.document.createElement('style'); style.innerHTML = `@page { size: auto; margin: 0mm; } body { margin: 0; }`; printWindow.document.head.appendChild(style); const rootEl = document.createElement('div'); printWindow.document.body.appendChild(rootEl); const root = ReactDOM.createRoot(rootEl); if (printer.format === 'Thermal') { root.render(<ThermalPrintableBill bill={bill} companyProfile={companyProfile} systemConfig={systemConfig} />); } else if (printer.format === 'A5') { root.render(<PrintableA5Bill bill={bill} companyProfile={companyProfile} systemConfig={systemConfig} />); } else { root.render(<PrintableBill bill={bill} companyProfile={companyProfile} systemConfig={systemConfig} />); } setTimeout(() => { printWindow.document.title = ' '; printWindow.print(); printWindow.close(); if (shouldReset) doReset(); }, 500); } else { alert("Please enable popups to print the bill."); if (shouldReset) doReset(); }
  ...
  const handleSaveBill = useCallback(async (shouldPrint: boolean) => { if (cart.length === 0) { alert(t.billing.cartEmpty); return; } if (systemConfig.maintainCustomerLedger && paymentMode === 'Credit' && !selectedCustomer) { alert("For credit bills, please select a registered customer."); return; } let savedBill: Bill | null = null; const isUpdate = isEditing && editingBill; const salesmanName = salesmen?.find(s => s.id === selectedSalesmanId)?.name; const billData: any = { date: isUpdate ? editingBill.date : new Date().toISOString(), customerName: customerName || t.billing.walkInCustomer, customerId: selectedCustomer ? selectedCustomer.id : null, doctorName: doctorName.trim(), items: cart, subTotal, totalGst, grandTotal, roundOff, paymentMode, salesmanId: selectedSalesmanId || null, salesmanName: salesmanName || null, }; if (isUpdate && onUpdateBill) { savedBill = await onUpdateBill(editingBill.id, { ...billData, billNumber: editingBill.billNumber }, editingBill); } else if (!isUpdate && onGenerateBill) { savedBill = await onGenerateBill(billData); } if (savedBill) { if (shouldPrint) { const defaultPrinter = systemConfig.printers?.find(p => p.isDefault); if (defaultPrinter) { executePrint(savedBill, defaultPrinter, true); } else { setBillToPrint(savedBill); setShouldResetAfterPrint(true); setPrinterModalOpen(true); } } else { if (startTimeRef.current) { const seconds = Math.floor((Date.now() - startTimeRef.current) / 1000); setOrderSeconds(seconds); } else { setOrderSeconds(0); } setLastSavedBill(savedBill); setShowOrderSuccessModal(true); setShowSuccessToast(true); setTimeout(() => setShowSuccessToast(false), 2500); } } else { console.error("Failed to save/update bill."); alert("There was an error saving the bill. Please try again."); } }, [cart, isEditing, editingBill, onUpdateBill, customerName, selectedCustomer, doctorName, subTotal, totalGst, grandTotal, roundOff, onGenerateBill, systemConfig, executePrint, t, paymentMode, selectedSalesmanId, salesmen]);
  ...
  const resetBillingForm = () => { setCart([]); setCustomerName(''); setSelectedCustomer(null); setDoctorName(''); setPaymentMode('Cash'); setSelectedSalesmanId(''); if (onCancelEdit && isEditing) { onCancelEdit(); } startTimeRef.current = null; setOrderSeconds(0); };

  return (
    <div className="p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 relative">
      {showSuccessToast && ( <div className="fixed top-20 right-6 z-50 bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg animate-fade-in-down flex items-center gap-2"> <CheckCircleIcon className="h-6 w-6" /> <span className="font-medium">Bill Saved Successfully!</span> </div> )}
                ...
                <div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg shadow-lg max-h-96 overflow-y-auto">
                    ...
                    {searchResults.map((product, productIndex) => (
                        ...
                        <ul className="pl-4 pb-2">
                            ...
                            // Visual Indicator for Near Expiry
                            const expiry = getExpiryDate(batch.expiryDate);
                            const ninetyDaysFromNow = new Date();
                            ninetyDaysFromNow.setDate(today.getDate() + 90);
                            const isNearExpiry = isPharmaMode && expiry > today && expiry <= ninetyDaysFromNow;

                            return (
                                <li key={batch.id} ref={isActive ? activeItemRef : null} className={`px-4 py-2 flex justify-between items-center transition-colors rounded-md mx-2 my-1 ${isActive ? 'bg-indigo-200 dark:bg-indigo-700' : 'hover:bg-indigo-50 dark:hover:bg-slate-600 cursor-pointer'}`} onClick={() => handleAddToCart(product, batch)} onMouseEnter={() => setActiveIndices({ product: productIndex, batch: batchIndex })}>
                                <div>
                                    {isPharmaMode && (
                                    <>
                                        <span className="text-slate-800 dark:text-slate-200">Batch: <span className="font-medium">{batch.batchNumber}</span></span>
                                        <span className={`text-sm ml-3 ${isNearExpiry ? 'text-amber-600 font-bold' : 'text-slate-600 dark:text-slate-400'}`}>
                                ...
                                <div className="flex items-center gap-4">
                                    <span className="text-slate-800 dark:text-slate-200"> MRP: <span className="font-medium">₹{batch.mrp.toFixed(2)}</span> {isPharmaMode && unitsPerStrip > 1 && <span className="text-xs">/S</span>} {isPharmaMode && unitsPerStrip > 1 && <span className="text-xs text-slate-500 dark:text-slate-400"> (₹{(batch.mrp / unitsPerStrip).toFixed(2)}/U)</span>} </span>
                ...
                <table className="w-full text-sm text-left text-slate-800 dark:text-slate-300">
                    ...
                    {cart.map(item => (
                        <tr key={item.batchId} className="bg-white dark:bg-slate-800 border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700">
                            <td className="px-2 py-3 font-medium text-slate-900 dark:text-white"> {item.productName} {isPharmaMode && item.isScheduleH && <span className="ml-1 text-xs font-semibold text-orange-600 dark:text-orange-500">(Sch. H)</span>} </td>
                            ...
                            {isPharmaMode && ( <td className="px-2 py-3"> <input ref={(el) => { cartItemStripInputRefs.current.set(item.batchId, el); }} type="text" inputMode="numeric" value={item.stripQty} onChange={e => updateCartItem(item.batchId, parseInt(e.target.value) || 0, item.looseQty)} onKeyDown={(e) => handleStripQtyKeyDown(e, item.batchId)} className={`w-14 p-1 text-center ${inputStyle}`} disabled={!item.unitsPerStrip || item.unitsPerStrip <= 1} /> </td> )}
                            <td className="px-2 py-3"> <input ref={(el) => { cartItemTabInputRefs.current.set(item.batchId, el); }} type="text" inputMode="numeric" value={item.looseQty} onChange={e => updateCartItem(item.batchId, item.stripQty, parseInt(e.target.value) || 0)} onKeyDown={(e) => handleTabQtyKeyDown(e, item.batchId)} className={`w-14 p-1 text-center ${inputStyle}`} /> </td>
                            <td className="px-2 py-3"> {isMrpEditable ? ( <input ref={(el) => { cartItemMrpInputRefs.current.set(item.batchId, el); }} type="number" step="0.01" value={item.mrp} onChange={(e) => updateCartItemDetails(item.batchId, { mrp: parseFloat(e.target.value) || 0, stripQty: item.stripQty, looseQty: item.looseQty })} onKeyDown={handleMrpKeyDown} className={`w-20 p-1 text-center ${inputStyle}`} /> ) : ( <span>₹{item.mrp.toFixed(2)}</span> )} </td>
                            <td className="px-2 py-3 font-semibold">₹{item.total.toFixed(2)}</td>
                            <td className="px-2 py-3"> <div className="flex items-center gap-2"> <button onClick={() => openEditItemModal(item)} className="text-blue-500 hover:text-blue-700" title="Edit Item"> <PencilIcon className="h-4 w-4" /> </button> <button onClick={() => removeFromCart(item.batchId)} className="text-red-500 hover:text-red-700" title="Remove Item"> <TrashIcon className="h-5 w-5" /> </button> </div> </td>
                        </tr>
                    ))}
                    </tbody>
                </table>
                ) : ( <div className="text-center py-10 text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-700/50 rounded-lg"> <p>{t.billing.cartEmpty}</p> </div> )}
             </div>
          </div>
        </Card>
      </div>
      ...
      <PrinterSelectionModal isOpen={isPrinterModalOpen} onClose={() => { setPrinterModalOpen(false); if (shouldResetAfterPrint) { resetBillingForm(); setShouldResetAfterPrint(false); } setBillToPrint(null); }} systemConfig={systemConfig} onUpdateConfig={handleUpdateConfig} onSelectPrinter={handlePrinterSelection} />
