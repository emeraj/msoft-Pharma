  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard title="Total Revenue" value={formatCurrency(stats.totalRevenue)} icon={<CashIcon className="h-6 w-6 text-green-800" />} color="bg-green-100" />
            <StatCard title="Total Bills" value={stats.totalBills.toLocaleString('en-IN')} icon={<ReceiptIcon className="h-6 w-6 text-blue-800" />} color="bg-blue-100" />
            <StatCard title="Average Bill Value" value={formatCurrency(stats.avgBillValue)} icon={<ChartBarIcon className="h-6 w-6 text-indigo-800" />} color="bg-indigo-100" />
            <StatCard title="Total Items Sold" value={stats.totalItemsSold.toLocaleString('en-IN')} icon={<CubeIcon className="h-6 w-6 text-yellow-800" />} color="bg-yellow-100" />
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card title="Monthly Sales (Last 12 Months)" className="lg:col-span-2">
                <div className="h-72 w-full p-4 flex justify-around items-end gap-2 sm:gap-4 bg-slate-50 rounded-lg">
                    {monthlySalesData.map((data, index) => (
                        <div key={index} className="flex-1 flex flex-col items-center h-full justify-end group">
                            <div 
                                className="w-full bg-indigo-400 rounded-t-md hover:bg-indigo-500 transition-all duration-300"
                    ...
                    <ul className="space-y-4">
                        {topSellingProducts.map((product, index) => (
                            <li key={index} className="flex items-center gap-4">
                                <div className="flex-shrink-0 h-8 w-8 flex items-center justify-center bg-slate-200 rounded-full font-bold text-slate-700">
                                    {index + 1}
                                </div>
